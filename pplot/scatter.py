...
def plot_with_label_color(
    X, y, shuffle=False, decompose=None, y_to_color=default_color_blind_colors, **kwargs
):
    ...
    if len(np.unique(y)) <= len(y_to_color):
        ...
        if shuffle:
            ...
            for yy in np.unique(y):
                lidx = y == yy
                mpl_plt.plot(
                    X[lidx, 0], X[lidx, 1], 'o', color=y_to_color[yy], **kwargs
                )
    else:
...
def df_scatter_plot(df=None, x=None, y=None, label=None, **kwargs):
    if df is None:
        if y is None and x is not None:
            x, y = x[:, 0], x[:, 1]
        assert (
            x is not None and y is not None and label is not None
        ), 'you need to specify x, y, and label'
        ...
        if len(df.columns) != 3:
            raise ValueError(
                "I can't (or rather won't) guess the label if there's not exactly 3 columns. "
    ...
    for i, this_label in enumerate(label_list):
        ...
        if colors:
            mpl_plt.scatter(
                ax, xvals, yvals, label=this_label, facecolor=colors[i], **kwargs
            )
        else:
...
def factor_scatter_matrix(df, factor, color_map=None, **kwargs):
    ...
    if isinstance(factor, str):
        ...
        df = df.drop(
            factor_name, axis=1
        )  # remove from df, so it doesn't get a row and col in the plot.
    else:
        df = df.copy()
    ...
    if len(classes) > len(color_map):
        raise ValueError(
            '''Too many groups for the number of colors provided.
                            ...
                            groups.'''.format(
                len(color_map), len(classes)
            )
        )

    colors = factor.apply(lambda gr: color_map[gr])
    scatter_matrix_kwargs = dict(
        {'figsize': (10, 10), 'marker': 'o', 'c': colors, 'diagonal': None},
