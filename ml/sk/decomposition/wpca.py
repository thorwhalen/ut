...
class WPCA(BaseEstimator, TransformerMixin):
    ...
    def _center_and_weight(self, X, weights, fit_mean=False):
        ...
        if weights is not None:
            X *= np.sqrt(weights)  # important change here!!!
        else:
    ...
    def _fit_precentered(self, X, weights):
        ...
        covar = np.multiply(X.shape[1], covar / np.sum(weights))

        covar[np.isnan(covar)] = 0

        # if all weights have integer values, set n_samples to be sum of weights
        if np.all(has_integer_value(weights)):
            n_samps = int(np.sum(weights, axis=0)[0])
        else:
            n_samps = X.shape[0]
        ...
        self.n_samples_ = n_samps
        self.n_components_ = n_components
        self.components_ = evecs[:, ::-1].T
        self.explained_variance_ = explained_variance_[::-1]
