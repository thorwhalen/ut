...
def score_classifier(X, y, clf, nfeats=None,
    ...
    for train, test in stratified_k_fold:
        ...
        for score_name, score_fun in scoring.iteritems():
    ...
    for score_aggreg_name, score_aggreg_fun in score_aggreg.iteritems():
        t = score_info.apply(score_aggreg_fun)
        t.set_axis(axis=0,
                   labels=map(lambda score_name: mk_aggreg_score_name(score_aggreg_name, score_name), t.index.values))
...
def __main__():
    ...
    clf_results = test_classifiers(X, y,
            ...
            n_features=range(1, np.shape(X)[1]),
            clfs=None,
            print_progress=False,
            score_to_plot=None)
