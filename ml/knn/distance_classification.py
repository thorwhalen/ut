...
def multiple_similarity_alignment(simil_matrix, y):
    ...
    for label in unique(y):
        ...
        (
            probability_of_alignment[label],
            number_of_aligned_pairs[label],
            total_num_of_error_possibilities[label],
        ) = similarity_alignment(
            mk_simil_ordered_same_class_pairs_bitmap(simil_matrix, class_idx)
        )
    aggregate_prob_of_alignment = np.sum(
        list(number_of_aligned_pairs.values())
    ) / np.sum(list(total_num_of_error_possibilities.values()))
...
def similarity_alignment(simil_ordered_same_class_pairs_bitmap):
    ...
    simil_ordered_same_class_pairs_bitmap = array(
        list(map(bool, simil_ordered_same_class_pairs_bitmap))
    )

    # The total number of error possibilities: The number of (0,1) or (1,0) pairs in
    total_num_of_error_possibilities = float(
        sum(simil_ordered_same_class_pairs_bitmap)
        * sum(~simil_ordered_same_class_pairs_bitmap)
    )
    ...
    number_of_aligned_pairs = total_num_of_error_possibilities - np.sum(
        cum_t[out_of_class_idx]
    )
    ...
    probability_of_alignment = (
        number_of_aligned_pairs / total_num_of_error_possibilities
    )
    return (
        probability_of_alignment,
        number_of_aligned_pairs,
...
def mk_simil_ordered_same_class_pairs_bitmap(simil_matrix, class_idx):
    ...
    band = band[:, permi]
    simil_ordered_same_class_pairs_bitmap = np.append(
        np.ones((k, k)), np.zeros((k, n - k)), axis=1
    ).astype(bool)
    ...
    simil_ordered_same_class_pairs_bitmap = simil_ordered_same_class_pairs_bitmap[mask][
        permi
    ]
