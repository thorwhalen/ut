...
class DistanceClf(object):
    def __init__(self,
        ...
        if not isinstance(dist_df, pd.DataFrame):  # if dist_df is not a dataframe, assume it's an ndarray, and make a df out of it
            self.dist_df = pd.DataFrame(data=dist_df, index=self.labels.index, columns=self.labels.index)
        else:
    ...
    def assert_record_alignment(self):
        ...
        """
        assert len(self.dist_df.index.values) == len(self.dist_df.columns), \
            "dist_df.index and dist_df.columns don't have the same number of elements"
        assert all(self.dist_df.index.values == self.dist_df.columns), \
            "dist_df.index and dist_df.columns are not aligned"
        if len(self.labels) > 0:
            assert len(self.dist_df.index.values) == len(self.labels.index.values), \
                "labels.index and dist_df.index don't have the same number of elementss"
            assert all(self.dist_df.index.values == self.labels.index.values), \
