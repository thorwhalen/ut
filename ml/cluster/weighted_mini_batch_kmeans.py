...
###############################################################################
# Extra functions taken out from cython's _k_means
...
def _centers_dense(X, labels, n_clusters, distances,
    ...
    if n_weights == 1:
        n_samples_in_cluster = \
            1. * np.bincount(labels, minlength=n_clusters)
    else:
        n_samples_in_cluster = \
            np.bincount(labels, weights=sample_weight, minlength=n_clusters)
...
def _labels_inertia(X, x_squared_norms, centers,
    ...
    # distances will be changed in-place
    # Temporary hack: sparse support not allowed and always precompute_distances...
    # if sp.issparse(X):
    ...
    #        X, x_squared_norms, centers, labels,
    #        distances=distances, sample_weight=sample_weight)
    return _labels_inertia_precompute_dense(X, x_squared_norms, centers, distances,
                                            sample_weight=sample_weight)
    #            X, x_squared_norms, centers, distances,
    #            sample_weight=sample_weight)
...
def _mini_batch_step(X, x_squared_norms, centers, counts,
    ...
    # cythonTODO: either implement it or remove the 4 lines below
    # if sp.issparse(X):
