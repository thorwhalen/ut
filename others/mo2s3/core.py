...
parser = argparse.ArgumentParser(
    description='Backup and restore MongoDB with Amazon S3.'
)
parser.add_argument(
    'action',
    help='configure/backup/restore FILENAME/delete FILENAME/list/drop',
    action='store',
)
parser.add_argument(
    '--host', help='Optional MongoDB Host', default=cfg.get('mongodb', 'host')
)
parser.add_argument(
    '-u',
    '--username',
    help='Optional MongoDB username',
    default=cfg.get('mongodb', 'username'),
)
parser.add_argument(
    '-p',
    '--password',
    help='Optional MongoDB password',
    default=cfg.get('mongodb', 'password'),
)
parser.add_argument('-d', '--db', help='Optional MongoDB database', default='')
parser.add_argument(
    '-c', '--collection', help='Optional MongoDB collection', default=''
)
parser.add_argument(
    '-b', '--bucket', help='AWS S3 bucket', default=cfg.get('aws', 's3_bucket')
)
parser.add_argument(
    '-k', '--folder', help='Folder (i.e. key prefix)', default=cfg.get('aws', 'folder')
)

parser.add_argument('-f', '--filename', help='File to delete/restore', default='')
parser.add_argument(
    '-a', '--access-key', help='AWS access key', default=cfg.get('aws', 'access_key')
)
parser.add_argument(
    '-s', '--secret-key', help='AWS secret key', default=cfg.get('aws', 'secret_key')
)
...
def main():
    ...
    if not args['access_key'] or not args['secret_key']:
        print(
            "S3 credentials not set, run 'mo2s3 configure' or specify --access-key/--secret-key, 'mo2s3 -h' to show the help"
        )
        sys.exit(0)
    ...
    if args['action'] == 'backup':
        ...
        try:
            ...
            print(
                'shutil.rmtree({dump_directory}) got an error (does {dump_directory} really exist?)'.format(
    ...
    # Restore action
        ...
        # Run mongorestore
        restore_cmd = (
            'mongorestore' + make_mongo_params(args) + ' mongodump_' + dump_date
        )
        if args['db']:
            restore_cmd += '/' + args['db']
            if args['collection']:
