...
class ParseSearchTerms:
    ...
    def process(self, search_term):
        try:
            try:  # getting the html for this search_term
                ...
                self.logger.warn(
                    msg='Could not get html for {}. '.format(
                        to_utf8_or_bust(search_term)
                    )
                    + e.message
                )
                return
            ...
            try:  # parsing the html for this search_term
                ...
                try:  # diagnose the parsed_result to decide where to go from here
                    ...
                    if (
                        diagnosis_string
                    ):  # non-empty diagnosis_string means diagnosis failed
                        self.logger.warn(
                            msg='Diagnosis failed for {} : {}'.format(
                                search_term, diagnosis_string
                            )
                        )
                    else:  # empty diagnosis_string means diagnosis succeeded
                        self.do_something_with_good_parsed_result(
                            search_term, parsed_result
                        )
                except Exception as e:
                    self.logger.warn(
                        msg='diagnose_parsed_result_exception for {} : {}'.format(
                            to_utf8_or_bust(search_term), e.message
                        )
                    )
                    return
            except Exception as e:
                self.logger.warn(
                    msg='parse exception for {} : {}.'.format(
                        to_utf8_or_bust(search_term), e.message
                    )
                )
                return
            ...
            try:  # getting info dict from parsed_result
                ...
                try:  # diagnose the info_dict to decide where to go from here
                    ...
                    self.logger.warn(
                        msg='diagnose_info_dict_exception for {} : {}.'.format(
                            to_utf8_or_bust(search_term), e.message
                        )
                    )
                    return
                ...
                self.logger.warn(
                    msg='get_info_exception for {} : {}.'.format(
                        to_utf8_or_bust(search_term), e.message
                    )
                )
                return
            ...
            self.logger.error(
                msg='untracked_exception for {} : {}.'.format(
                    to_utf8_or_bust(search_term), e.message
                )
            )
    ...
    def html_to_parsed_result(self, search_term, html):
        try:  # parsing the html for this search_term
            ...
            self.logger.warn(
                msg='parse_exception for {} : {}.'.format(
                    to_utf8_or_bust(search_term), e.message
                )
            )
            return None
    ...
    def parsed_to_info_dict_process(self, search_term):
        try:
            try:  # getting the parsed_dict for this search_term
                ...
                self.logger.warn(
                    msg='get_parsed_dict_exception for {} : {}.'.format(
                        to_utf8_or_bust(search_term), e.message
                    )
                )
                try:  # getting the html for this search_term
                    ...
                    self.logger.warn(
                        msg='get_html_exception for {} : {}.'.format(
                            to_utf8_or_bust(search_term), e.message
                        )
                    )
                    return
    ...
    @classmethod
    def for_cloud(
        cls, html_folder, parsed_results_folder, info_dict_folder, log_filename
    ):
        instance = cls()
    ...
    def _set_local_data_source(self, html_folder):
        ...
        html_data_accessor = DataAccessor(
            relative_root=html_folder,
            extension='',  # '.html'
    ...
    def _set_parse_and_get_info(self):
        ...
        self.get_info = (
            parse_gresult_info_retriever.GResultInfoRetriever.for_nres_words_domains().get_info
        )
    ...
    def _set_local_actions(self, parsed_results_folder, info_dict_folder, log_filename):
        self.do_something_with_good_parsed_result = SaveDictLocally(
            save_folder=parsed_results_folder
        ).save
        self.do_something_with_good_info_dict = SaveDictLocally(
            save_folder=info_dict_folder
        ).save
    ...
    def _set_cloud_actions(self, parsed_results_folder, info_dict_folder, log_filename):
        self.do_something_with_good_parsed_result = SaveDictToS3(
            save_folder=parsed_results_folder
        ).save
        self.do_something_with_good_info_dict = SaveDictToS3(
            save_folder=info_dict_folder
        ).save
