...
def add_query_fanout_scores(query_report_df):
    ...
    ad_group_fanout = mk_query_fanout_scores(
        query_report_df, target='ad_group', statVars='impressions', keep_statVars=False
    )
    ad_group_fanout = daf_ch.ch_col_names(
        ad_group_fanout,
        ['ad_group_imps_freq_fanout_ratio', 'ad_group_count_fanout_ratio'],
        ['impressions_freq_fanout_ratio', 'impressions_count_fanout_ratio'],
    )
    destination_fanout = mk_query_fanout_scores(
        query_report_df,
        target='destination',
        statVars='impressions',
        keep_statVars=False,
    )
    destination_fanout = daf_ch.ch_col_names(
        destination_fanout,
        ['destination_imps_freq_fanout_ratio', 'destination_count_fanout_ratio'],
        ['impressions_freq_fanout_ratio', 'impressions_count_fanout_ratio'],
    )
    query_report_df = query_report_df.merge(
        ad_group_fanout, on=['search_term', 'ad_group']
    )
    query_report_df = query_report_df.merge(
        destination_fanout, on=['search_term', 'destination']
    )
    return query_report_df


def mk_query_fanout_scores(
    query_report_df, target='ad_group', statVars='impressions', keep_statVars=False
):
    ...
    if target not in query_report_df.columns:
        if target == 'destination':
            ...
            raise ValueError(
                "the dataframe doesn't have the column %s and I don't know how to make it"
                % target
            )
    return daf_diagnosis.mk_fanout_score_df(
        query_report_df,
        fromVars=['search_term'],
        toVars=target,
        statVars=statVars,
        keep_statVars=keep_statVars,
    )


def add_destination(query_report_df):
    assert (
        'ad_group' in query_report_df.columns
    ), 'you must have the variable ad_group to infer destination'
    # get the destination as the substring before the first |
    query_report_df['destination'] = query_report_df['destination'] = query_report_df[
        'ad_group'
    ].apply(lambda x: re.match('[^|]*', x).group(0))
    return query_report_df
...
def similarity_with_travel_domains(domain_list):
    ...
    return len(set(domain_list).intersection(set(travel_domain_list))) / float(
        len(domain_list)
    )
...
def mk_term_count_from_google_results(gresults):
    ...
    df.columns = ['count']
    df = df.sort(
        columns=['count'], ascending=False
    )  # TODO: Take out sorting at some point since it's unecessary (just for diagnosis purposes)
    return df
