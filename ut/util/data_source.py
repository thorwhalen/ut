...
def for_global_local():
    ...
    f_dict = {
        ...
        'khan_parse_search_terms_log': os.path.join(
            dir_dict['log'], 'khan_parse_search_terms.log'
        ),
    }
    d_dict = {'khan_term_map': os.path.join(f_dict['khan_term_map'])}
    instance = DataSource(
...
class DataSource:
    def __init__(
        ...
        if mother_root:
            if mother_root[-1] != '/':
                mother_root = mother_root + '/'
        self.dir = Directories(mother_root=mother_root, dir_dict=dir_dict)
        ...
        if self.location == LOCATION_LOCAL:
            ...
            for k, v in list(self.a.__dict__.items()):
                ...
                if not os.path.exists(root_directory):
                    print(
                        '!!!Non existent accessor root directory: %s' % root_directory
                    )
        ...
        if location == LOCATION_LOCAL:
            for k, v in list(a_dict.items()):
                if isinstance(v, str):
                    ...
                    self.__setattr__(
                        k,
                        pfile_accessor.for_local(
                            relative_root=os.path.join(mother_root, v)
                        ),
                    )
