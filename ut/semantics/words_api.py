...
class Words(object):
    ...
    def __init__(self, api_key=None, pretty=None):
        if api_key is None:
            if 'WORDSAPI_KEY' not in os.environ:
                print(
                    'You need an api key to use this. Get it at https://www.mashape.com/'
                )
                print(
                    'At the time of this writing, exactly here: https://market.mashape.com/wordsapi/wordsapi'
                )
                print(
                    'I suggest you enter this key in an environmental variable under the name WORDSAPI_KEY'
                )
                print("Then, you won't need to specify it every time")
                raise ValueError(
                    'No api_key given or found in environmental variable name WORDSAPI_KEY'
                )
            else:
    ...
    def random(self):
        ...
        """
        query = requests.get(
            self._base_url, params={'random': 'true'}, headers=self._auth_headers
        )
