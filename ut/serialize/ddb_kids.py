...
class DDBKids(Dynamo):
    ...
    def set_max_kid(self, account_name, kid):
        ...
        return self.table.put_item(
            {'account_name': account_name, 'kid': kid}, overwrite=True
        )
    ...
    def get_max_kid(self, account_name):
        ...
        if res['kid']:
            ...
            self.logger.warn(
                'Creating a new (max) KID entry for account {} because it did not yet exist in ddb_kids'.format(
                    account_name
                )
            )
    ...
    def truncate_table(self):
        ...
        """
        assert (
            self.table.table_name == 'test_kids'
