...
def more_like(
    ...
    **more_like_this_kwargs
    ...
    more_like_this_query = {'fields': similarity_fields, 'like': like_these}
    more_like_this_query.update(**more_like_this_kwargs)

    return {'query': {'more_like_this': more_like_this_query}}
...
def match_field(field, value=None):
    if value is None:
        assert isinstance(
            field, dict
        ), 'you must specify a field and value, or field={field: value}'
...
def min_and_max_of_field(field='timestamp'):
    return {
        ...
        'query': {'match_all': {}},
        'aggs': {
            'min_of_field': {'min': {'field': field}},
            'max_of_field': {'max': {'field': field}},
        },
    }


def field_range(field='timestamp', gte=0, lte=1e6):
    return {'query': {'range': {field: {'gte': gte, 'lte': lte}}}}


def field_gte(field='timestamp', gte=0):
    return {'query': {'range': {field: {'gte': gte}}}}
