...
def parse_organic_result(search_ires_li_instance):
    ...
    if div_class_s:
        ...
        if orp_table:
            ...
            table_tds = [
                x for x in table_tds if x.text
            ...
            if len(table_tds) >= 1:
                ...
                if span:
                    d = dict(
                        d,
                        **{'td0_span_text': span.get_text(separator='\n', strip=True)}
                    )
...
def mk_tag_dict(soup):
    ...
    for t in to_expand:
        if t in d:
            d = dict(d, **extract_tag_dict_from_node(d[t], t))
...
def extract_tag_dict_from_node(node, dict_spec):
    if dict_spec == 'root':
        ...
        d = {'appbar': appbar, 'center_col': center_col, 'rhscol': rhscol}
        ...
        d = {
            'search': extract_section(
                node, attrs={'id': 'search'}, name='div'
            )  # organic search results
        }
    elif dict_spec == 'extrares':
        d = {
            'brs': extract_section(
                node, attrs={'id': 'brs'}, name='div'
            )  # related searches
        }
        ...
        d = {
            ...
            'rhsvw vk_rhsc': extract_section(
                node, attrs={'id': 'rhsvw vk_rhsc'}, name='div'
            ),  # hotel finder (specific)
        }
    else:
        d = {}
