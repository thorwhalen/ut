...
def get_rand_function(
    params, rand_class=scipy.stats.norm, default_mean=0.0, default_std=None
):
...
class UserModel:
    def __init__(
        ...
        if isinstance(hour_of_visit, ndarray) and len(hour_of_visit) == 24:
            hour_of_visit = random.choice(
                a=list(range(len(hour_of_visit))), p=hour_of_visit
            )
        self.hour_of_visit = get_rand_function(
            params={'loc': hour_of_visit, 'scale': 2.0}, rand_class=RandomHour
        )

        inactivity_lag_days = inactivity_lag_days or scipy.stats.poisson.rvs(
            mu=7, loc=1
        )
        self.inactivity_lag_days = get_rand_function(
            params={'mu': inactivity_lag_days}, rand_class=scipy.stats.poisson
        )

        visit_pages = visit_pages or scipy.stats.poisson.rvs(mu=3, loc=1)
        self.visit_pages = get_rand_function(
            params={'mu': visit_pages, 'loc': 1}, rand_class=scipy.stats.poisson
        )

        seconds_on_page = seconds_on_page or scipy.stats.lognorm.rvs(s=1, loc=25)
        self.seconds_on_page = get_rand_function(
            params={'s': 1, 'loc': seconds_on_page, 'scale': seconds_on_page / 2.0},
    ...
    def next_session_data(self):
        current_date_midnight = datetime.fromordinal(
            self.current_date.date().toordinal()
        )  # round date to day level
        self.current_date = current_date_midnight + timedelta(
            self.inactivity_lag_days() + self.hour_of_visit() / HOURS_PER_DAY
        )
