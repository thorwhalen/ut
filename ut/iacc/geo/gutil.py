...
class GeoMongoDacc(object):
    ...
    def find_nearest_one(self, lat, lon, **kwargs):
        return self.collection.find_one(
            self.nearest_neighbors_query(
                coordinate_field=self.coordinate_field, lat=lat, lon=lon
            ),
            **kwargs
        )
    ...
    def find_in_circle(self, lat, lon, max_km, **kwargs):
        return self.collection.find(
            self.in_circle_query(
                coordinate_field=self.coordinate_field, lat=lat, lon=lon, max_km=max_km
            ),
            **kwargs
        )
    ...
    def in_circle_query(cls, coordinate_field, lat, lon, max_km):
        return {
            coordinate_field: {
                '$within': {'$center': [[lon, lat], max_km / degree_kms]}
            }
        }
