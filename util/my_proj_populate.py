...
def gen_key_content_and_description(source, excluded_descriptions=(), excluded_keys=(), verbose=True):
...
class mk_key_to_pkg_name_from_collection:
    ...
    def __init__(self, collection_of_pkgs):
        ...
        assert len(set(self.collection_of_pkgs)) == self.collection_size, \
    ...
    def __call__(self, k):
        if k not in self.assigned:
            if self.cursor >= self.collection_size:
                raise IndexError("I already assigned all collection_of_pkgs items (see them in self.assigned), "
                                 "so can't assign {k}")
...
def mk_and_deploy_new_pkgs_from_existing_modules(
        source, new_pkg_dir, key_to_pkg_name, root_url,
        excluded_descriptions=(), excluded_keys=(),
    ...
    if not isinstance(key_to_pkg_name, Callable) and isinstance(key_to_pkg_name, Iterable):
        key_to_pkg_name = mk_key_to_pkg_name_from_collection(key_to_pkg_name)

    target_store = QuickTextStore(new_pkg_dir)

    for k, description, v in gen_key_content_and_description(source, excluded_descriptions, excluded_keys, verbose):
        if k not in excluded_keys:
            try:
                ...
                target_store[os.path.join(pkg_name, pkg_name, target_module_name)] = v  # copy contents to init module
                ...
                go(pkg_dir, skip_git_commit=True, version='0.0.1', twine_upload_options_str=twine_upload_options_str)
                if verbose:
